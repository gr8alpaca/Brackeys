...
func visibility_check() -> void:
	...
	assert(is_inside_tree(), "visibility_check")
...
func _get_camera_2d() -> Camera2D:
	...
	assert(is_inside_tree(), "_get_camera_2d viewfinder not in tree")
...
#		Has Camera
		if camera.get_children().size() > 0:
			for cam_child in camera.get_children():
				...
				if pcam_host:
					assert(is_inside_tree(), "_check_camera")
...
func _instantiate_node(root: Node, node: Node, name: String) -> void:
	node.set_name(name)
	root.add_child(node)
	assert(is_inside_tree(), "_instantiate_node")
	node.set_owner(get_tree().get_edited_scene_root())


func _set_viewfinder(root: Node, editor: bool) -> void:
	assert(is_inside_tree(), "_set_viewfinder")
